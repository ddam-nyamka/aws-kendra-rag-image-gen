...
if prompt := st.chat_input("Enter your message"):
    ...
    with st.chat_message("assistant"):
        ...
        assistant_prompt = prompt_base.generate_answer_kendra_prompt(kendra_items_prompt)
        response = bedrock.answer(history, assistant_prompt)
...
with st.sidebar:
    ...
    history_list = [
        f"{record['role']}: {record['content'][:20]}..." for record in st.session_state.messages
    ]
    st.write(history_list)
    
    st.subheader("Found items Kendra:")
    kendra_list = [
        f"{record['DocumentTitle']}: {record['Content'][:20]}..." for record in st.session_state.kendra_items
    ]
